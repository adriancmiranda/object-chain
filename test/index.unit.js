...
const match = objectChain({
	...
	controlChar: (self, value) => `${self}\\c${value}`,
	...
	size: (self, value) => `${self}{${value>>>0}}`,
	atLeast: (self, value) => `${self}{${value>>>0},}`,
	atMost: (self, value) => `${self}{,${value>>>0}}`,
	group: (self, value) => `${self}(${value})`,
	range: (self, min, max) => `${self}{${min>>>0},${max>>>0}}`,
